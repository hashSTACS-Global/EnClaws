...
export function resolveCommandAuthorization(params: {
  ...
  // In multi-tenant mode, JWT-authenticated users (CommandAuthorized via
  // gateway) are treated as owners so they can access owner-only tools
  // like cron without being in the legacy allowFrom list.
  const senderIsOwner = Boolean(matchedSender) || (!!ctx.TenantId && commandAuthorized);
