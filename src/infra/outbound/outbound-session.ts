...
function resolveFeishuSession(
  ...
  if (!typeExplicit) {
    if (idLower.startsWith("ou_") || idLower.startsWith("on_")) {
      ...
      // ID does not match any known Feishu format (ou_, on_, oc_) and has
      // no explicit type prefix (user:, dm:, group:, chat:).  This commonly
      // happens in multi-tenant setups where the session store records an
      // internal web-console user name as `lastTo`.  Reject early to avoid
      // a guaranteed 400 from the Feishu API.
      return null;
    }
