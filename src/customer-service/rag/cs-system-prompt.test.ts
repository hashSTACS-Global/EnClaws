...
import { buildCSSystemPrompt, DEFAULT_CS_BASE_PROMPT } from "./cs-system-prompt.js";
import type { MemorySearchResult } from "../../memory/types.js";

describe("buildCSSystemPrompt", () => {
  it("includes role and behavior rules", () => {
    const prompt = buildCSSystemPrompt({ basePrompt: DEFAULT_CS_BASE_PROMPT, knowledgeChunks: [] });
  ...
  it("includes knowledge chunks when provided", () => {
    ...
    const prompt = buildCSSystemPrompt({ basePrompt: DEFAULT_CS_BASE_PROMPT, knowledgeChunks: chunks });
  ...
  it("includes visitor name when provided", () => {
    const prompt = buildCSSystemPrompt({ basePrompt: DEFAULT_CS_BASE_PROMPT, knowledgeChunks: [], visitorName: "张三" });
