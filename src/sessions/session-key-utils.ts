...
export function parseAgentSessionKey(
  ...
  let raw = (sessionKey ?? "").trim().toLowerCase();
  if (!raw) {
    return null;
  }
  // Strip tenant prefix "t:{tenantId}:" if present so the parser sees
  // the canonical "agent:{agentId}:..." form.
  const tenantMatch = raw.match(/^t:[^:]+:(agent:.+)$/);
  if (tenantMatch) {
    raw = tenantMatch[1];
  }
