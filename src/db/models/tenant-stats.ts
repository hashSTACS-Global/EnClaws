...
export async function getTenantSummary(tenantId: string) {
  if (getDbType() === DB_SQLITE) {
    return sqliteStats.getTenantSummary(tenantId);
  }
  ...
  const [
    ...
    query("SELECT COUNT(*) as c FROM tenant_agents WHERE tenant_id = $1 AND is_active = true", [
      tenantId,
    ]),
    query(
      "SELECT COUNT(DISTINCT agent_id) as c FROM llm_interaction_traces WHERE tenant_id = $1 AND created_at >= NOW() - INTERVAL '30 days'",
      [tenantId],
    ),
    query("SELECT COUNT(*) as c FROM tenant_channels WHERE tenant_id = $1", [tenantId]),
    query("SELECT COUNT(*) as c FROM tenant_channels WHERE tenant_id = $1 AND is_active = true", [
      tenantId,
    ]),
    query("SELECT COUNT(*) as c FROM tenant_channel_apps WHERE tenant_id = $1", [tenantId]),
    query(
      "SELECT COALESCE(SUM(jsonb_array_length(models)), 0) as c FROM tenant_models WHERE tenant_id = $1 AND is_active = true",
      [tenantId],
    ),
    query("SELECT COUNT(*) as c FROM tenant_models WHERE tenant_id = $1 AND is_active = true", [
      tenantId,
    ]),
    query(
      "SELECT COUNT(DISTINCT COALESCE(union_id, id::text)) as c FROM users WHERE tenant_id = $1 AND status = 'active'",
      [tenantId],
    ),
    query(
      "SELECT COUNT(DISTINCT user_id) as c FROM llm_interaction_traces WHERE tenant_id = $1 AND created_at >= NOW() - INTERVAL '30 days'",
      [tenantId],
    ),
    query(
      "SELECT COALESCE(SUM(input_tokens + output_tokens), 0) as c FROM usage_records WHERE tenant_id = $1",
      [tenantId],
    ),
    query(
      "SELECT COALESCE(SUM(input_tokens + output_tokens), 0) as c FROM usage_records WHERE tenant_id = $1 AND recorded_at >= DATE_TRUNC('month', NOW())",
      [tenantId],
    ),
    query(
      "SELECT COALESCE(SUM(input_tokens + output_tokens), 0) as c FROM usage_records WHERE tenant_id = $1 AND recorded_at >= DATE_TRUNC('day', NOW())",
      [tenantId],
    ),
    query(
      "SELECT COALESCE(SUM(input_tokens + output_tokens), 0) as c FROM usage_records WHERE tenant_id = $1 AND recorded_at >= DATE_TRUNC('month', NOW()) - INTERVAL '1 month' AND recorded_at < DATE_TRUNC('month', NOW())",
      [tenantId],
    ),
  ...
  const admin = await query(
    "SELECT display_name FROM users WHERE tenant_id = $1 AND role = 'owner' LIMIT 1",
    [tenantId],
  );
  ...
  return {
    ...
    agents: {
      total: pInt(agentTotal.rows[0]?.c),
      active: pInt(agentActive.rows[0]?.c),
      active30d: pInt(agentActive30d.rows[0]?.c),
    },
    channels: {
      total: pInt(channelTotal.rows[0]?.c),
      active: pInt(channelActive.rows[0]?.c),
      apps: pInt(appCount.rows[0]?.c),
    },
...
export async function getTenantTokenTrend(tenantId: string, days = 7) {
  if (getDbType() === DB_SQLITE) {
    return sqliteStats.getTenantTokenTrend(tenantId, days);
  }
...
export async function getTenantTokenRank(
  tenantId: string,
  period: "all" | "month" | "today" = "all",
  limit = 5,
) {
  if (getDbType() === DB_SQLITE) {
    return sqliteStats.getTenantTokenRank(tenantId, period, limit);
  }
...
export async function getTenantLlmStats(
  tenantId: string,
  period: "all" | "month" | "today" = "all",
) {
  if (getDbType() === DB_SQLITE) {
    return sqliteStats.getTenantLlmStats(tenantId, period);
  }
  ...
  const [turnsRes, avgRes, errorRes, modelRes] = await Promise.all([
    query(`SELECT COUNT(DISTINCT turn_id) as c FROM llm_interaction_traces WHERE ${where}`, [
      tenantId,
    ]),
    query(
...
export async function getTenantChannelDistribution(tenantId: string) {
  if (getDbType() === DB_SQLITE) {
    return sqliteStats.getTenantChannelDistribution(tenantId);
  }
...
export async function getTenantRecentTraces(tenantId: string, limit = 10) {
  if (getDbType() === DB_SQLITE) {
    return sqliteStats.getTenantRecentTraces(tenantId, limit);
  }
