...
import { CHANNEL_ICON_MAP as _CHANNEL_ICON_MAP } from "../../../constants/channels.ts";
...
export class TenantOverviewView extends LitElement {
  ...
  private async loadAll() {
    ...
    try {
      ...
      this.trend =
        trend.status === "fulfilled" ? ((trend.value as { items: TrendItem[] }).items ?? []) : [];
      this.rank = rank.status === "fulfilled" ? (rank.value as RankData) : null;
      this.llmStats = llm.status === "fulfilled" ? (llm.value as LlmStats) : null;
      this.channelDist =
        channelDist.status === "fulfilled"
          ? ((channelDist.value as { channels: ChannelDist[] }).channels ?? [])
          : [];
      this.recentTraces =
        traces.status === "fulfilled"
          ? ((traces.value as { traces: RecentTrace[] }).traces ?? [])
          : [];
    } catch {
  ...
  private fmtTime(iso: string | null): string {
    ...
    if (diff < 60_000) {
      return t("tenantOverview.justNow");
    }
    if (diff < 3600_000) {
      return t("tenantOverview.minutesAgo", { n: String(Math.floor(diff / 60_000)) });
    }
    if (diff < 86400_000) {
      return t("tenantOverview.hoursAgo", { n: String(Math.floor(diff / 3600_000)) });
    }
  ...
  private async loadTrend() {
    try {
      ...
      this.trend = ((res as Record<string, unknown>).items ?? []) as TrendItem[];
    } catch {
      this.trend = [];
    }
  }
  ...
  private updateTrendChart() {
    ...
    this.trendChart.setOption({
      ...
      series: [
        {
          name: t("tenantOverview.inputToken"),
          type: "line",
        ...
        {
          name: t("tenantOverview.outputToken"),
          type: "line",
    ...
    this.llmPieChart.setOption(
      {
        ...
        tooltip: {
          ...
          formatter: (p: Record<string, unknown>) =>
            `${String(p.name)}<br/>${t("tenantOverview.calls")}: ${String(p.value)}<br/>${String(p.percent)}%`,
        },
        legend: {
          orient: "vertical",
          right: 10,
    ...
    this.channelPieChart.setOption(
      {
        ...
        tooltip: {
          ...
          textStyle: { color: this.cssVar("--text"), fontSize: 12 },
          formatter: (p: Record<string, unknown>) =>
            `${String(p.name)}<br/>${t("tenantOverview.count")}: ${String(p.value)}<br/>${String(p.percent)}%`,
        },
        legend: {
          orient: "vertical",
          right: 10,
  ...
  private renderRankList(items: Array<{ name: string; sub?: string; tokens: number }>) {
    if (items.length === 0) {
      return html`<div class="rank-empty">${t("tenantOverview.noData")}</div>`;
    }
  ...
  render() {
    if (this.loading) {
      return html`<div class="loading">${t("tenantOverview.loading")}</div>`;
    }
    if (!this.summary) {
      return html`<div class="empty">${t("tenantOverview.noData")}</div>`;
    }
    ...
    return html`
      ...
      ${(() => {
        const monthDiff =
          s.tokens.lastMonth > 0
            ? ((s.tokens.month - s.tokens.lastMonth) / s.tokens.lastMonth) * 100
            : 0;
        const monthDiffText =
          s.tokens.lastMonth > 0 ? `${monthDiff >= 0 ? "+" : ""}${monthDiff.toFixed(1)}%` : "-";
      ...
      <div class="section">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.5rem">
          ...
          <div class="period-tabs">
            <button class="period-tab ${this.period === "7d" ? "active" : ""}" @click=${() => {
              this.period = "7d";
            }}>${t("tenantOverview.last7d")}</button>
            <button class="period-tab ${this.period === "30d" ? "active" : ""}" @click=${() => {
              this.period = "30d";
            }}>${t("tenantOverview.last30d")}</button>
          </div>
        </div>
        ${
          this.trend.length > 0
            ? html`
                <div class="trend-chart chart-container"></div>
              `
      ...
      <div class="section">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem">
          ...
          <div class="period-tabs">
            <button class="period-tab ${this.rankPeriod === "all" ? "active" : ""}" @click=${() => {
              this.rankPeriod = "all";
            }}>${t("tenantOverview.periodAll")}</button>
            <button class="period-tab ${this.rankPeriod === "month" ? "active" : ""}" @click=${() => {
              this.rankPeriod = "month";
            }}>${t("tenantOverview.periodMonth")}</button>
            <button class="period-tab ${this.rankPeriod === "today" ? "active" : ""}" @click=${() => {
              this.rankPeriod = "today";
            }}>${t("tenantOverview.periodToday")}</button>
          </div>
        </div>
        <div class="three-col">
      ...
      <div class="section">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem">
          ...
          <div class="period-tabs">
            <button class="period-tab ${this.llmPeriod === "all" ? "active" : ""}" @click=${() => {
              this.llmPeriod = "all";
            }}>${t("tenantOverview.periodAll")}</button>
            <button class="period-tab ${this.llmPeriod === "month" ? "active" : ""}" @click=${() => {
              this.llmPeriod = "month";
            }}>${t("tenantOverview.periodMonth")}</button>
            <button class="period-tab ${this.llmPeriod === "today" ? "active" : ""}" @click=${() => {
          ...
          <div>
            ${
              (this.llmStats?.modelDistribution?.length ?? 0) > 0
                ? html`
                    <div class="llm-pie-chart chart-sm"></div>
                  `
      ...
      <div class="two-col">
        <div class="section">
          ...
          ${
            this.channelDist.length > 0
              ? html`
                  <div class="channel-pie-chart chart-sm"></div>
                `
              : html`<div class="chart-sm" style="display:grid;place-items:center;color:var(--muted);font-size:0.8rem">${t("tenantOverview.noData")}</div>`
          }
        </div>
        <div class="section">
          ...
          ${
            this.recentTraces.length > 0
              ? html`
            <div class="activity-list">
              ${this.recentTraces.slice(0, 10).map(
                (tr) => html`
